...
import { CrisisResourcesLink } from "@/components/CrisisResourcesLink";
...
export default function ReportPage() {
  ...
  async function handleSubmit() {
    ...
    try {
      ...
      if (photos.length > 0) {
        // The edit code returned above proves ownership of this report and is
        // required by /api/upload — a reportId alone is not sufficient to
        // attach media. This doesn't change what the user sees or does; the
        // code is simply threaded through the same request instead of the
        // upload trusting reportId on its own.
        await Promise.all(
          photos.map((file) => {
            const formData = new FormData();
            formData.append("file", file);
            formData.append("reportId", data.reportId);
            formData.append("editCode", data.editCode);
