...
export default function MapPage() {
  ...
  async function handleConfirm(reportId: string) {
    try {
      ...
      const data = await res.json();
      // V2: reconcile with the server's actual returned count (from the
      // atomic increment RPC added in Phase 1) rather than blindly
      // optimistic +1. Two tabs/devices confirming the same report at
      // nearly the same time would otherwise drift from the true count
      // until the next full refetch — using the server's confirmCount
      // directly means the displayed number is always correct immediately,
      // not just eventually.
      if (res.ok && typeof data.confirmCount === "number") {
        setReports((prev) =>
          prev.map((r) =>
            r.id === reportId ? { ...r, confirm_count: data.confirmCount } : r
          )
        );
      }
    } catch {
      // Silent failure here matches the rest of this app's pattern for
      // non-critical background actions (e.g. photo upload) — a failed
      // confirm shouldn't interrupt map browsing with an error state.
    }
  }
