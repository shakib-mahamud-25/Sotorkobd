...
export function SeverityStep({
  ...
  return (
    <div className="animate-fade-in space-y-6">
        ...
        <div className="w-full px-1">
          <div className="relative">
            ...
            <input
              ...
              // V2 accessibility fix: this slider previously had no
              // accessible name and no value description beyond a bare
              // number — a screen reader announced only "slider, 3" with
              // nothing conveying that 1 means minor and 5 means severe.
              // aria-label gives the control a name; aria-valuetext
              // replaces the announced value with a phrase built from the
              // same two anchor labels already shown visually (minor/
              // uncomfortable at 1, severe/dangerous at 5), so a screen
              // reader user gets the same information a sighted user gets
              // from the color and position, not just a number.
              aria-label={t("report.severity.title")}
              aria-valuetext={t("report.severity.valueText").replace("{n}", String(value))}
            />
          </div>
      ...
      <div className="flex justify-between text-xs text-[var(--color-text-secondary)]" aria-hidden="true">
